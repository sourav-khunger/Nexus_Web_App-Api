    ...
    try {
      ...
      $user->user_social_id = $decoded->data->user_social_id;
       if($user->doesSocialIdExist()){
          $user->hidden_by = $decoded->data->user_id;
          $user->hidden_user =trim($data->user_id);
          $user->hidden_id=$user->hidden_by < $user->hidden_user ?$user->hidden_by.$user->hidden_user : $user->hidden_user.$user->hidden_by;
          if($user->hideUser())
          {
              $response["success"]=1;
              $response["message"]="User hidden Successfully.";
          }
		else{
		     $response["success"]=0;
