...
public function getUserIDWithSocialIDs(){
    $query = "SELECT user_id FROM ". $this->table_user ." WHERE user_social_id='" . htmlspecialchars(strip_tags($this->user_social_id)) . "'";
        // prepare query statement
        $stmt = $this->conn->prepare($query);
        $stmt->execute();
...
public function deleteUserAllPurposes()
    ...
    public function DoesDeviceExist()
    {
       $query = "SELECT * FROM " . $this->table_devices . " WHERE device_id='" . htmlspecialchars(strip_tags($this->device_id)) . "' AND user_id='" . htmlspecialchars(strip_tags($this->user_id)) . "'";
        // prepare query statement
        $stmt = $this->conn->prepare($query);
        $stmt->execute();
        $num = $stmt->rowCount();
 ...
 public function getUserDataByUserID()
  ...
  public function saveMeeting(){
    ...
    public function getUserRegistrationStep()
    {
        $query = "SELECT user_registration_step as step FROM " . $this->table_user . " WHERE   user_social_id='" . htmlspecialchars(strip_tags($this->user_social_id)) . "'";
        // prepare query statement
        $stmt = $this->conn->prepare($query);
        $stmt->execute();
        return $stmt;
    ...
    public function insertProfileInfo(){
    	...
    	skills='" . $this->skills . "' WHERE user_social_id='" . htmlspecialchars(strip_tags($this->user_social_id)) . "'";
        $stmt = $this->conn->prepare($query);
       if ($stmt->execute()) {
    ...
    public function addNewDevice(){
        ...
        $query="UPDATE " . $this->table_devices . " SET 
    	push_token='" . htmlspecialchars(strip_tags($this->push_token)) ."' WHERE device_id='" . htmlspecialchars(strip_tags($this->device_id)) . "' AND
    	user_id='" . htmlspecialchars(strip_tags($this->user_id)) . "'";
    	$stmt = $this->conn->prepare($query);
    	 if ($stmt->execute()) {
        return true;
...
public function updateStep(){
     $query = "UPDATE " . $this->table_user . " SET 
      user_registration_step='1' WHERE user_social_id='" . htmlspecialchars(strip_tags($this->user_social_id)) . "'";
      $stmt = $this->conn->prepare($query);
        if ($stmt->execute()) {
        return true;
        } else {
    ...
    {
        $query = "UPDATE " . $this->table_user . " SET recess_status = '" . $this->status . "' WHERE user_id = '" . $this->user_id . "'";
        $stmt = $this->conn->prepare($query);
         if ($stmt->execute()) {
            return true;
        } else {
